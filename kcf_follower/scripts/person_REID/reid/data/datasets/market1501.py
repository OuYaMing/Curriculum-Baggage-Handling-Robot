...
class Market1501(BaseImageDataset):
    ...
    """
    def __init__(self, root=u'query', verbose=True, **kwargs):
    ...
    def _process_dir(self, dir_path, relabel=False):
        ...
        img_paths = glob.glob(osp.join(dir_path, u'*.jpg'))
        ...
        for img_path in img_paths:
            # pid: 每个人的标签编号 3
            # _  : 摄像头号 1
            pid, _ = imap(int, pattern.search(img_path).groups())
        ...
        pid2label = dict((pid, label) for label, pid in enumerate(pid_container))

        dataset = []
        for img_path in img_paths:
            pid, camid = imap(int, pattern.search(img_path).groups())
